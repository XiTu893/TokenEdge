...
import { getModelById, PC_MODELS } from './model/ModelConfig';
...
function findAvailablePort(startPort: number, maxPort: number): Promise<number> {
...
ipcMain.handle('download-model', async (event, downloadUrls: string[], modelId: string) => {
  ...
  downloader.on('progress', (data: any) => {
    mainWindow?.webContents.send('download-progress', { ...data, modelId });
  });

  downloader.on('source-change', (data: any) => {
  ...
  downloader.on('waiting', (data: any) => {
...
ipcMain.handle('start-service', async (event, config: ServiceConfig) => {
  if (isServerRunning) {
...
ipcMain.handle('open-chat', async (event, config: any) => {
...
ipcMain.handle('start-openclaw', async (event, config: any) => {
